...
export class UIController {
  ...
  private async handleDownloadPdf() {
    ...
    // Ненавязчивый опрос точности печати
    this.showFeedbackToast();
  }

  /**
   * Ненавязчивый Apple HIG баннер обратной связи (Feedback Loop)
   */
  private showFeedbackToast() {
    try {
      if (sessionStorage.getItem('stickerfit_feedback_given')) return;
    } catch {}

    let banner = document.getElementById('feedbackToast');
    if (!banner) {
      banner = document.createElement('div');
      banner.id = 'feedbackToast';
      banner.className = 'feedback-toast';
      document.body.appendChild(banner);
    }

    banner.innerHTML = `
      <div class="feedback-toast-inner">
        <span class="feedback-toast-text">${t('feedbackPrompt')}</span>
        <div class="feedback-toast-actions">
          <button type="button" class="btn-feedback-yes" id="btnFeedbackYes">${t('feedbackYes')}</button>
          <button type="button" class="btn-feedback-no" id="btnFeedbackNo">${t('feedbackNo')}</button>
          <button type="button" class="btn-feedback-close" id="btnFeedbackClose" aria-label="${t('feedbackDismiss')}">✕</button>
        </div>
      </div>
    `;
    ...
    const btnYes = banner.querySelector('#btnFeedbackYes') as HTMLButtonElement;
    const btnNo = banner.querySelector('#btnFeedbackNo') as HTMLButtonElement;
    const btnClose = banner.querySelector('#btnFeedbackClose') as HTMLButtonElement;

    btnYes?.addEventListener('click', () => {
      trackEvent({ name: 'feedback_response', data: { satisfied: true } });
      try {
        sessionStorage.setItem('stickerfit_feedback_given', 'true');
      } catch {}
      if (banner) {
        banner.innerHTML = `<div class="feedback-toast-inner"><span class="feedback-toast-text">${t('feedbackThankYou')}</span></div>`;
        setTimeout(() => banner?.classList.remove('visible'), 3000);
      }
    });

    btnNo?.addEventListener('click', () => {
      trackEvent({ name: 'feedback_response', data: { satisfied: false } });
      try {
        sessionStorage.setItem('stickerfit_feedback_given', 'true');
      } catch {}
      if (banner) {
        banner.innerHTML = `
          <div class="feedback-toast-inner">
            <span class="feedback-toast-text">${t('feedbackReportIssue')}:</span>
            <a href="https://github.com/sefga/stickerfit/issues/new?template=print_accuracy.md" target="_blank" rel="noopener noreferrer" class="feedback-issue-link">GitHub Issues ↗</a>
            <button type="button" class="btn-feedback-close" id="btnFeedbackClose2" aria-label="${t('feedbackDismiss')}">✕</button>
          </div>
        `;
        banner.querySelector('#btnFeedbackClose2')?.addEventListener('click', () => {
