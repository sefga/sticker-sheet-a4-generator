...
export const translations = {
  ru: {
    ...
    faq1Q: 'Почему размеры напечатанных наклеек отличаются от заданных в программе?',
    faq1A: 'Наиболее частая причина — включенное масштабирование страницы в диалоге печати. Убедитесь, что в настройках принтера выбран масштаб 100% («Реальный размер»), а не «Подогнать под размер страницы».',
    faq2Q: 'Как подготовить стикеры под плоттерную резку?',
    faq2A: 'Включите метки реза (Cut marks) и задайте зазор между стикерами от 2 до 4 мм. Для цветных стикеров с заливкой до края добавьте вылет Bleed 1–2 мм.',
    faq3Q: 'Безопасно ли загружать конфиденциальные макеты?',
    faq3A: 'Да, на 100%. StickerFit работает исключительно в вашем браузере. Исходные файлы и сгенерированные PDF никуда не отправляются и не сохраняются на внешних серверах.',
    faq4Q: 'Какое разрешение должно быть у исходного изображения?',
    faq4A: 'Для четкой полиграфической печати рекомендуется разрешение не менее 300 DPI. Панель параметров автоматически рассчитывает эффективный DPI для заданного вами физического размера.',
  },
